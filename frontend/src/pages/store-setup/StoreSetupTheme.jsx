...
export default function StoreSetupTheme({ isRTL, t, onNext, onBack }) {
  const { storeId, selectedPlan, selectedTheme, setSelectedTheme } = useStoreSetup();
  ...
  const handleNext = async () => {
    ...
    setError('');
    try {
      await axiosInstance.post('/api/store-setup/select-theme', { store_id: storeId, theme: selectedTheme });
      onNext();
    } catch (err) {
        ...
        {THEMES.map((theme) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 h-14 rounded-lg border border-gray-200 overflow-hidden" style={{ background: colors.length ? `linear-gradient(135deg, ${colors[0]} 0%, ${colors[1] || colors[0]} 100%)` : '#f3f4f6' }} />
              {!enabled && (
