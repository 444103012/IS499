/**
 * LandingPage.jsx - Home / Marketing Page (IS498)
 * ------------------------------------------------
 * EN: First page at "/". StoreLaunch landing: navbar (logo, Features, Pricing, language toggle,
 *     Get Started), hero, 3 features, pricing (monthly/yearly, 3 plans), footer. Implements
 *     localization (Table 31 Switch Language): Arabic/English via i18n, RTL support. Text from
 *     t() so all strings come from en.json / ar.json.
 * AR: الصفحة الأولى على "/". الصفحة الرئيسية: شريط التنقل، البطل، 3 ميزات، الأسعار (شهري/سنوي،
 *     3 خطط)، تذييل. تنفذ تغيير اللغة (جدول 31): عربي/إنجليزي عبر i18n ودعم RTL. النص من t() من en.json / ar.json.
 */
...
const LandingPage = () => {
  ...
  /** Switch between Arabic and English; also set document dir and lang for RTL/accessibility */
  ...
  /** When language changes, ensure the page direction (RTL/LTR) and lang attribute are correct */
  useEffect(() => {
  ...
  /** Data for the 3 feature cards: each has a translation key and an icon name */
  const featuresList = [
  ...
  /** Pricing plans: basic, pro, advanced */
  const plans = [
  ...
  return (
    <div className="min-h-screen bg-white" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* ----- Navbar: logo, desktop links, language button, Get Started, mobile menu toggle ----- */}
      <nav className="sticky top-0 z-50 bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`flex justify-between items-center h-14 md:h-16 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            {/* Mobile: hamburger button to open/close menu */}
            <button
              type="button"
          ...
          {isMobileMenuOpen && (
            <div className={`md:hidden py-4 border-t border-gray-100 ${isRTL ? 'text-right' : 'text-left'}`}>
              ...
              <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="block mt-2 px-4 py-2 bg-storelaunch-dark text-white rounded-lg font-medium text-center">
                {t('nav.login')}
              </Link>
            </div>
          )}
      ...
      {/* ----- Hero: headline, subheadline, two buttons (Get Started + Learn more) ----- */}
      ...
      {/* ----- Features: 3 cards (easy store, customize, inventory) ----- */}
      ...
      {/* ----- Pricing: monthly/yearly toggle, then 3 plan cards ----- */}
      <section id="pricing" className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Toggle: Monthly vs Yearly (billing period state) */}
          ...
          {/* Plan cards: price and features come from translations (pricing.plans.basic/pro/advanced) */}
